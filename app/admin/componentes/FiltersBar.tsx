...
export default function FiltersBar(props: Props) {
  ...
  return (
    <div className="w-full rounded-3xl border bg-white/90 p-4 flex flex-wrap gap-4 items-end">
      ...
      {props.sortOptions && (
        <div>
          <label className="block text-xs text-neutral-500 mb-1">Ordenar</label>
          <select
            value={props.sort || ""}
      ...
      {props.onPerPageChange && typeof props.perPage === "number" && (
        <div>
          <label className="block text-xs text-neutral-500 mb-1">Por pagina</label>
          <select
      ...
      {!controlled && (
        <>
          <button onClick={applyUrl} className="btn btn-primary text-sm px-4 py-2">Aplicar</button>
          <button onClick={() => { setQLocal(""); router.push(window.location.pathname); }} className="text-sm underline">
            Limpiar
          </button>
        </>
      )}
